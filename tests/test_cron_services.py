"""Tests for cron service registration (HA-free)."""
...
@pytest.mark.asyncio
async def test_cron_services_registered() -> None:
    ...
    assert integration.SERVICE_CRON_ADD in service_names
    assert integration.SERVICE_CRON_REMOVE in service_names
    assert integration.SERVICE_CRON_RUN in service_names
